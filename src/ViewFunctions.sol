...
library ViewFunctions {
     ...
     */
    function getBalanceOfFundraiser(FundraiserStorage.State storage self, uint256 _fundraiserId)
        external
        view
        returns (uint256)
    {
     ...
     */
    function getUserContribution(FundraiserStorage.State storage self, address _user, uint256 _fundraiserId)
        external
        view
        returns (uint256)
    {
     ...
     */
    function getFundraiser(FundraiserStorage.State storage self, uint256 _fundraiserId)
        external
        view
        returns (
            address owner,
     ...
     */
    function getAmountInBaseUnits(FundraiserStorage.State storage self, uint256 _amount)
        external
        view
        returns (uint256)
    {
     ...
     */
    function checkAllowance(FundraiserStorage.State storage self, address user, address contractAddress)
        external
        view
        returns (uint256)
    {
    ...
    function getApprovalInstructions(FundraiserStorage.State storage self) external view returns (string memory) {
        return string(
            abi.encodePacked(
                ...
                FundraiserLogic.addressToString(self.usdcAddress),
                "\n",
     ...
     */
    function batchGetFundraisers(FundraiserStorage.State storage self, uint256 _fromId, uint256 _count)
        external
        view
        returns (FundraiserStorage.BatchResponse memory response)
    {
     ...
     */
    function getContractStatusBasic(FundraiserStorage.State storage self, bool isPaused)
        external
        view
        returns (bool, bool, bool, uint256)
    {
        return (isPaused, self.aaveEnabled, self.emergencyWithdrawalsEnabled, self.emergencyReservePercentage);
    }
     ...
     */
    function getContractStatusBalances(FundraiserStorage.State storage self)
        external
        view
        returns (uint256, uint256, uint256, bool)
    {
        ...
        return (totalDeposits, contractUsdcBalance, availableYield, isAccountingSafe);
    }
}
