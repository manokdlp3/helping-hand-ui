...
contract FundraiserFactory is FundraiserStorage, Ownable, ReentrancyGuard, Pausable {
    ...
    function emergencyUserWithdraw(uint256 _fundraiserId) external nonReentrant {
        ...
        uint256 withdrawAmount = FundraiserLogic.processEmergencyUserWithdraw(
            theFundraiser,
            state,
            msg.sender,
            _fundraiserId
     ...
     */
    function _getContractStatusInternal(bool isPaused) private view returns (
        bool,
        bool,
        bool,
        uint256,
    ...
    function getApprovalInstructions() external view returns (string memory) {
        return string(abi.encodePacked(
            ...
            "3. Enter USDC address: ", FundraiserLogic.addressToString(state.usdcAddress), "\n",
     ...
     */
    function canPerformEmergencyWithdrawal(uint256 _fundraiserId, address _user) external view returns (bool possible, uint256 maxAmount) {
        if (_fundraiserId >= state.fundraiserEventCounter) revert InvalidInput(1);
        
        Fundraiser storage theFundraiser = state.idToFundraiserEvent[_fundraiserId];
        return FundraiserLogic.canPerformEmergencyWithdrawal(
            theFundraiser,
            state,
            _user,
            _fundraiserId
