...
library AaveFunctions {
    ...
    function depositToAave(FundraiserStorage.State storage self, address callingContract) internal returns (bool) {
        ...
        uint256 amountToDeposit = AaveHelpers.calculateDepositAmount(
            self.usdcAddress,
            callingContract,
        ...
        bool success = AaveHelpers.depositToAave(
            self.usdcAddress,
            self.aavePoolAddress,
            amountToDeposit,
            callingContract
        );
    ...
    function withdrawFromAave(FundraiserStorage.State storage self, uint256 _amount) internal returns (uint256) {
        ...
        uint256 actualWithdrawn = AaveHelpers.withdrawFromAave(
            self.usdcAddress,
            self.aavePoolAddress,
            amountToWithdraw,
            address(this)
    ...
    function emergencyWithdrawFromAave(FundraiserStorage.State storage self) external returns (uint256) {
        ...
        try IPool(self.aavePoolAddress).withdraw(
            self.usdcAddress,
            type(uint256).max,
            address(this)
     ...
     */
    function distributeYield(FundraiserStorage.State storage self, address _recipient, uint256 _amount) external returns (bool) {
    ...
    function getAvailableYield(FundraiserStorage.State storage self) internal view returns (uint256) {
        return AaveHelpers.getAvailableYield(
            self.aUsdcAddress,
            address(this),
            self.totalDeposited
     ...
     */
    function checkAaveAccountingAccuracy(FundraiserStorage.State storage self) 
        internal view returns (bool) {
        
        (uint256 internalBalance, uint256 aaveBalance, int256 difference) = 
            AaveHelpers.getAaveBalanceAccuracy(
                self.aUsdcAddress,
                address(this),
                self.totalDeposited,
                self.aaveEnabled
